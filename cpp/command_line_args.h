...
template <typename T>
inline bool CommandLineArgs::GetCmdLineArgument(const char *arg_name, T &val)
{
	std::map<std::string, std::string>::iterator itr;
	if ((itr = pairs.find(arg_name)) != pairs.end())
	{
...
template <>
inline bool CommandLineArgs::GetCmdLineArgument<char *>(const char *arg_name, char *&val)
{
	std::map<std::string, std::string>::iterator itr;
	if ((itr = pairs.find(arg_name)) != pairs.end())
	{
